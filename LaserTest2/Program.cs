...
namespace LaserTest2
{
    internal class Program
    {
        static unsafe void ShowDevDetails(DEVICEINFO* Info, uint IndentSize)
        {
        ...
        static unsafe void Main()
        {
            ...
            {
                uint hBoxes = LsrBox.hBoxes[0]; // MUST change to ulong for 64-bit targets
                ...
                if (rc == ALSRCALRC.ALSRCAL_NO_ERROR)
                {
                    ...
                    if (rc == ALSRCALRC.ALSRCAL_NO_ERROR)
                    {
                        ...
                        for (uint i = 0; i < 10; i++)
                            Console.WriteLine("    Sample " + i.ToString() + " = " + MyData.LsrPos[i].ToString());
                    }
